...
func TestParseV3WithOneFile(t *testing.T) {
	...
	mysqlCon.HealthCheck = &ecs.HealthCheck{
		...
		Command:  aws.StringSlice([]string{"CMD-SHELL", "curl -f http://theecscliisawesome.com || exit 1"}),
...
services:
  ...
  mysql:
    ...
    healthcheck:
        test: curl -f http://theecscliisawesome.com || exit 1
        interval: 1m45s
        timeout: 15s
        retries: 5`
...
func verifyContainerConfig(t *testing.T, expected, actual adapter.ContainerConfig) {
	...
	if expected.HealthCheck != nil && actual.HealthCheck != nil {
		...
		assert.Equal(t, aws.Int64Value(expected.HealthCheck.Interval), aws.Int64Value(actual.HealthCheck.Interval), "Expected healthcheck interval to match")
		assert.Equal(t, aws.Int64Value(expected.HealthCheck.Retries), aws.Int64Value(actual.HealthCheck.Retries), "Expected healthcheck retries to match")
		assert.Equal(t, aws.Int64Value(expected.HealthCheck.StartPeriod), aws.Int64Value(actual.HealthCheck.StartPeriod), "Expected healthcheck start_period  to match")
		assert.Equal(t, aws.Int64Value(expected.HealthCheck.Timeout), aws.Int64Value(actual.HealthCheck.Timeout), "Expected healthcheck timeout to match")
	} else {
